...
public class EmaSupercomputer {
    static class Plus {
        ...
        boolean overlaps(Plus p2, int r, int c) {
            ...
            for (int i = p2.x - p2.len; i <= p2.x + p2.len; i++) {
                if (grid[p2.y][i] == Boolean.TRUE) {
                    return true;
                }

            }
            for (int i = p2.y - p2.len; i <= p2.y + p2.len; i++) {
                if (grid[i][p2.x] == Boolean.TRUE) {
                    return true;
                }
            }
            return false;
    ...
    public static int twoPluses(String[] grid, int m) {
        ArrayList<Plus> list = new ArrayList<Plus>();

        for (int y = 0; y < grid.length; y++) {
            for (int x = 0; x < m; x++) {
                if (grid[y].charAt(x) != 'B')
                    findPlus(x, y, grid.length, m, grid, list);
            }
        }
        ...
        for (int i = 0; i < list.size() - 1; i++) {
            ...
            for (int j = i + 1; j < list.size(); j++) {
                ...
                if (sum > max && !p1.overlaps(p2, grid.length, m)) {
                    max = sum;
                }
            }
        }
